...
public class Formulario implements ActionListener{
	...
	public void mostrarInformacion(){
		...
		for(int i=0;i<alumnos.size();i++)
			txtaInformacion.append(alumnos.get(i).toCSV()+"\n");
	}
	...
	public void guardarRegistroArchivo(Alumno a){
		try {
			BufferedWriter bw = new BufferedWriter(new FileWriter("alumnos.csv",true));
			bw.write(a.toCSV()+"\n");
			bw.close();
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void leerInformacion(){
		try {
			BufferedReader bf = new BufferedReader (new FileReader("alumnos.csv"));
			String linea = "";
			while((linea=bf.readLine())!=null){
				//System.out.println(linea);
				String partes[] = linea.split(",");
				ArrayList<String> jornadas = new ArrayList<String>();
				String partesJornadas[] = partes[5].split(":");
				for (int i=0;i<partesJornadas.length-1;i++)
					jornadas.add(partesJornadas[i]);

				Alumno a = new Alumno(
							partes[0],
							partes[1],
							partes[2],
							new Carrera(Integer.parseInt(partes[3]), partes[4]),
							jornadas
				);
				alumnos.add(a);
			}
